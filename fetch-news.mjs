...
// Ajuste os feeds e palavras-chave conforme seu ramo
const FEEDS = [
...
const HORAS_JANELA = 48; // considera notícias publicadas nas últimas 48h
...
function dentroDaJanela(item) {
  if (!item.isoDate) return true; // se não tem data, deixa passar pro filtro do modelo
...
async function coletarItens() {
  ...
  // remove duplicados por URL e limita quantidade antes de mandar pro modelo
